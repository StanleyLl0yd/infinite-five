...
for old, new in [
    ...
    ("      const threshold = this.pointerType === 'touch' ? 10 : 5;", "      const threshold = event.pointerType === 'touch' ? 10 : 5;")
]:
    if canvas_text.count(old) != 1:
        raise SystemExit(f'canvas-board.ts: expected one match for {old!r}')
    canvas_text = canvas_text.replace(old, new)
