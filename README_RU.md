...
Текущая версия исходников: **0.3.0** · Web + PWA · GitHub Pages
...
- усиленная тактика уровней Средне и Сложно;
- Эксперт объединяет атакующий, защитный и пространственный анализ кандидатов с более глубоким ограниченным minimax-поиском;
...
- на телефоне важные кнопки больше не скрываются, а переходят в компактный режим с иконками;
...
| Сложно | Шире перебирает тактические кандидаты, ищет вилки и оценивает лучшие ответы соперника |
| Эксперт | Пересекает результаты нескольких профилей оценки, использует псевдослучайный порядок близких вариантов, ищет двойные угрозы и глубже просчитывает дерево ответов с alpha-beta отсечениями |

Поиск Эксперта ограничен временем и шириной дерева: задача — сделать его существенно сильнее, не превращая каждый ход в долгую паузу и не подвешивая мобильный интерфейс.
...
src/
├── game/
│   ├── ai.ts              оценка AI и уровни сложности
...
Ближайший фокус после этого этапа развития:

- дальнейшая настройка Эксперта на реальных позициях игроков;
- доступность и клавиатурное управление;
- профилирование производительности очень длинных партий;
- более полезная, но по-прежнему минимальная локальная статистика;
