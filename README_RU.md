...
Текущий опубликованный релиз: **v0.4.0** · Web + PWA · GitHub Pages. В исходниках также заложена кроссплатформенная основа Tauri 2 для будущих нативных сборок; v0.4.0 не означает публикацию Android, macOS или iOS-версии.
...
Та же TypeScript/Vite-реализация подготавливается к нативной упаковке через **Tauri 2**. Нативные пакеты содержат frontend внутри приложения и не используют GitHub Pages как основной интерфейс. Для нативной сборки PWA-генерация отключается, поэтому обновление Service Worker в браузере отделено от обновлений APK/приложения через магазин или другой канал распространения.

Планируемый порядок: сначала Android через **RuStore**, затем macOS с возможностью прямого распространения, а iOS и дополнительные Android-магазины — по мере появления необходимого доступа. Kotlin/Swift/Rust-код должен использоваться только для платформенных интеграций: lifecycle, share, haptics, подпись, store updates и подобное. Правила игры и AI остаются общими.
...
- unit- и regression-тесты Vitest для поля, истории, обмена партиями, локализации, тактики AI и AI Lab;
...
Отдельный усиленный workflow повторно проверяет сборку и публикует GitHub Pages после изменений в `main`. Перед нативным релизом дополнительно должна успешно собираться целевая нативная упаковка.
...
В **v0.4.0** завершены: клавиатурное управление и доступность, reduced motion, оптимизация отрисовки длинных партий, ограниченная локальная история, повтор из истории и более полезная локальная статистика AI.

Следующие приоритеты:

- сформировать и проверить общую Tauri 2 основу, не разделяя игровой код по платформам;
- подготовить Android-сборку, подпись, нативные интеграции и публикацию в RuStore;
